...
- Captured same-day configuration and simulation activity in this repo:
  - Wokwi assets updated (`Wokwi/diagram.json`, `Wokwi/libraries.txt`, `Wokwi/wokwi-project.txt`, `Wokwi/wokwi.toml`, `Wokwi/uno_main.cpp`).
  - PlatformIO and source updates reflected in `platformio.ini` and `src/nano_main.cpp`.
...
- Aligned hardware flashing behavior so the Nano environment builds the parallel LCD firmware source at `src/nano_main.cpp`.
